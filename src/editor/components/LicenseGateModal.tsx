...
export default function LicenseGateModal({
	...
	// Show the manual key field only on request — unless there's no checkout configured, in which
	// case entering a key is the only way to unlock, so start there.
	const [showKeyEntry, setShowKeyEntry] = useState(!CHECKOUT_URL);
	...
	return (
		...
		>
			<p className="modal-lead">
				Building and previewing are free — publishing to your own website needs a one-time license. A secure checkout
				opens right here, and you’ll be unlocked automatically after paying. Your work is saved.
			</p>
			...
			{showKeyEntry ? (
				...
				<p className="modal-note">
					...
					<button type="button" className="btn-link" onClick={() => setShowKeyEntry(true)}>
						Enter it here
					</button>
				</p>
			)}
