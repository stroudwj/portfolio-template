...
| E8 | `resume.label` — `Résumé`, **all 14** | empty | `needs a control` — **renderer half done** (spec 36 chunk 1); the label *field* belongs to chunk 4 | No store action writes `content.resume.label`, and `PortfolioPage` falls back to `content.resume?.label \|\| 'Résumé'`. Invisible while the résumé URL is empty, so it never reached the harness output; it becomes an undeletable, unrenameable link label the moment an artist attaches a résumé. Fix: a label field beside the résumé upload. **Chunk 1 removed the renderer's half:** `PortfolioPage` now takes the label as written and drops the link entirely when it is empty (a link with no text is nothing to announce); a document from before the field existed, with no label at all, still gets the historical `Résumé` so its link keeps working. The harness's fields pass now empties `resume.label` against a real résumé URL, so the fallback cannot come back. |
