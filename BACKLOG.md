...
## 36. Promote starter one-offs into real formats; make every starter string deletable — `queued` (unblocked 2026-08-11: spec 35 merged. AUDIT CORRECTIONS: the Footer and Text bullets below are MOOT — every block type and footer arrangement the starters use is already reachable in the UI, no new formats needed. The real work is the audit's four file-disjoint chunks, runnable in parallel: (1) renderer fallbacks in src/portfolio/ — 404 shell copy, image-viewer title hole, works-on-paper slug/label fallbacks, resume.label; (2) form-block fields — ContactForm's undeletable Send message/Required/unavailable strings, the only schema-touching chunk; (3) font menu — 18 stacks used by 13 starters absent from FONT_OPTIONS, blank-doc artists can't reach the bundled faces; land this chunk BEFORE spec-14 batch 3; (4) small missing fields — carousel counter etc. Editor floors are by design, don't "fix": last section and home page are undeletable; footerColumns []→undefined is normalized in the store (latent hardening only). E1/E3/E5-aria are William's product decisions, ask before changing.)
