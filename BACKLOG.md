...
## 36. Promote starter one-offs into real formats; make every starter string deletable — `merged` (2026-08-11 onto integration/specs-14r-19, built as four parallel Opus chunks in one orchestrated wave. HARNESS_STRICT=1 now passes for all fourteen starters + blank: everything that survives emptying is the accepted PRODUCT_CHROME list in tests/starter-empty.test.ts (404 shell, hamburger, lightbox function label, form honeypot + accessible name, carousel arrows/aria) — a new survivor is a regression, not a baseline. Chunk highlights: renderer fallbacks removed (lightbox label hole, sub-page slug — the real culprit was ensurePageBlocks() re-backfilling cleared page labels on every parse, fixed in content-schema.ts; footer !! guard; résumé link drops when label emptied); ContactForm submit/required/unavailable strings are schema fields defaulting to the old literals (schema 5 unchanged, old drafts byte-identical); font menu names all 25 stacks the starters/presets use incl. the ten bundled OFL faces installable from a blank doc via fontChoicePatch() (tests/font-menu.test.ts is the batch-3 guard); résumé label + sub-page card label controls added. Orchestrator fixes at merge: vitest now excludes .claude-worktrees/** (stale worktree suites were polluting npm test), accepted chrome consolidated into PRODUCT_CHROME. Still William's: E1 404 copy, E3 hamburger label, E5 carousel aria wording. Spec-14 batch 3 is now unblocked on every gate except William's go.) renderer fallbacks in src/portfolio/ — 404 shell copy, image-viewer title hole, works-on-paper slug/label fallbacks, resume.label; (2) form-block fields — ContactForm's undeletable Send message/Required/unavailable strings, the only schema-touching chunk; (3) font menu — 18 stacks used by 13 starters absent from FONT_OPTIONS, blank-doc artists can't reach the bundled faces; land this chunk BEFORE spec-14 batch 3; (4) small missing fields — carousel counter etc. Editor floors are by design, don't "fix": last section and home page are undeletable; footerColumns []→undefined is normalized in the store (latent hardening only). E1/E3/E5-aria are William's product decisions, ask before changing.)
...
## 37. The catalog becomes the face: retire legacy starters, real-screenshot picker, template names — `merged` (2026-08-11 onto integration/specs-14r-19, NOT pushed to main — William reviews the product-site changes and the works-on-paper retirement first (one revert restores it). Built on `worktree-spec-37-catalog-face`, branched from integration/specs-14r-19; not merged, not pushed.

**A — done, wider than the three named.** Retired **five** pre-catalog registry entries, not three: `painter`, `photographer`, `sculptor` plus the two the spec's audit clause catches — `works-on-paper` (the corkboard pinboard) and `illustrator-designer` (the never-cleared `awaiting-permission` placeholder). All five date from the same 2026-08-05 commit; the registry is now exactly the ten spec-14 starters. **Judgement call to confirm: if William wants Works on paper kept, it is one revert of its recipe + JSON.** Repointed references: `MobileDoor`'s phone demo doc (painter → conservatory, with a first-cleared-template fallback), the template-studio dashboard cover, and every test that used a legacy starter as its fixture (mostly → `clearing`, whose one-images-block home is the nearest structural analogue to painter's, plus `promenade` for the multi-group-home case and `masthead`/`signal` for second-template cases). **Sample artwork was deliberately NOT removed**: `sample-artwork.ts` and `public/assets/starters/{painter,photographer,sculptor,works-on-paper}/` stay exactly as they were, because still-room and signal hang some of those Met images *and* because a draft an artist built from a legacy starter still points at them — new test `keeps retired-starter media cataloged so legacy drafts still render` (41 samples, bytes + checksums + rights + the Monet profile portrait) is the guard. Discipline routing: all five tags still route — sculpture now leads with **still-room alone** (the two legacy sculpture-tagged starters are gone), asserted explicitly in `template-apply.test.ts` rather than by weakening the old "≥2 looks" bar; batch 3 restores the pair. Spec-35 harness is green including `HARNESS_STRICT=1` (25 tests: blank + ten structure + ten fields), template-studio round-trip green, `validateStarterCatalog()` clean.

**B — done.** `scripts/capture-template-shots.mjs` starts its own Astro dev server, opens each starter through the dev-only template studio (`/editor?template-studio=starter:<id>`), enters the fullscreen preview — the view labelled "shown exactly as your published site" — screenshots the preview iframe at a fixed 1440×900 ×2, and re-encodes to a 720×450 webp in-browser via `OffscreenCanvas` (no image dependency added). Regen command: **`node scripts/capture-template-shots.mjs`** (or `… <id> <id>` for a subset) — documented in the script header and in CLAUDE.md; output is byte-stable across runs. Ten shots, ~330 KB total, in `public/assets/starters/shots/<id>.webp`. `starterShotUrl()` in `templates.ts` is the single accessor; the intake tiles, the picker cards, the classic starter grid, the template-studio dashboard, the landing band and /templates all use it. Cards now read **template name + design tagline**; the intake's first question changed from "What do you make?" to "Pick a starting design", and discipline survives only as ordering (`templatesForDiscipline`) and as the quiet corner label on the start-screen card.

**C — done.** `/examples` → **`/templates`** (`src/pages/templates/[...path].astro`), cards = generated shot + name + tagline + "Open in editor" (`/editor?template=<id>`, a new deep link `StartScreen` consumes once and strips from the URL). The old `/examples` route survives as a `noindex` redirect stub so nothing already linked 404s. Renamed everywhere: landing nav + hero CTA ("Browse templates"), landing band (`#examples` → `#templates`, four template cards), footers on learn/guide/faq/legal, the learn article's proof paragraph, and the sitemap (`/templates`; the stub stays out). `src/lib/examples.ts` survives as the single made-with proof line (stroud.xyz) under the templates grid. **Gap for spec 5:** there are still no live per-template demo sites — one real published site exists (stroud.xyz) and the cards therefore link into the editor, not to a hosted demo.

**Capture gotcha worth knowing (fixed, but note it before touching the script).** Walking the preview iframe's images from the parent frame (`iframe.contentDocument.images`) *crashes the Chromium tab* on **still-room** — repeatably, only that starter, only in the fullscreen preview; it is the one starter with a marquee. The script now uses a fixed 5s settle instead of polling images, and all ten capture in one clean run. Two smaller traps baked into the script: Astro bumps the dev port when one is taken (so the origin is parsed from its output, not assumed), and the editor's unsaved-draft `beforeunload` can stall `browser.close()` (so the close is raced against a 5s timer and the server is killed by process group).

Gate: `npm test` 405 passed, `npm run check` 0 errors, `npm run build:product` succeeds (17 pages), `runtime:check` clean, `.hangwork` manifests regenerated and committed.)
...
## 38. First-party funnel analytics for hangwork.art (UGC attribution + drop-off) — `merged` (2026-08-12 onto integration/specs-14r-19. Recon confirmed zero product-site instrumentation; new build. Shared step-enum + ref-sanitizer contract in `oauth-proxy/lib/funnel-contract.js` (+`.d.ts`) imported by both Worker and browser; beacon `src/lib/funnel.ts` gated on IS_PRODUCT_SITE with sessionStorage once-per-step guard and sticky `?ref=`/utm tag; seven call sites incl. publish in PublishModal's `setPhase('success')` (the only place success is known) and a new `openLicenseGate()` all three paywall openers route through. Worker: `POST /funnel/event` (anonymous, allowlisted, strict enum, server re-sanitize, body cap) → `funnel:<YYYY-MM>` KV; `GET /admin/funnel` behind requireAdmin with 6 periods + per-month polar_orders counts; funnel panel in AdminApp. Two browser-found bugs: sendBeacon always sends credentials-include so a JSON Blob preflights and dies against the shared cors() helper — beacon sends CORS-safelisted `text/plain`, Worker parses; and enum-order rows printed publish above paywall as a −129% drop — dashboard has its own row order and dashes non-nested steps. Verified live: `/?ref=TT-Video-3` → `{"step":"landing","ref":"tt-video-3"}`, sticky across to `/editor`, untagged reload sends nothing; `HANGWORK_IS_PRODUCT_SITE=false npm run build` emits `/funnel/event` nowhere in dist/ (tree-shaken, stronger than the staticgen grep test). 14 new tests in tests/funnel.test.ts; gate green 419/419, manifest clean on regen. **Not live until William runs `wrangler deploy` in oauth-proxy/ AND pushes main** — until the Worker ships the beacon 404s harmlessly. Accepted limits documented in code: KV rmw race loses occasional counts; >200 refs/step/month fold into `other`; counts are tab sessions, not people. Spec 37 rebase over Landing.astro is one appended script block, trivial.)
